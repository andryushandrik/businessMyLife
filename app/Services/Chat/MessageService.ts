...
export default class MessageService {
	public static async paginate(conversationId: Conversation['id'], config: ApiValidator['schema']['props']): Promise<ModelPaginatorContract<Message>> {
		...
		try {
			return await conversation.related('messages').query().getViaPaginate(config)
		} catch (err: any) {
			Logger.error(err)
	...
	public static async createWithoutTopic(
		{ text }: MessageWithoutTopicValidator['schema']['props'],
		payload: MessageCreateWithoutTopicPayload,
	...
	public static async createWithOfferTopic(
		...
		try {
			conversation = await ConversationService.getWithOfferTopic(payload.offerId, createPayload, {
				trx,
			})
		} catch (err: Err | any) {}
