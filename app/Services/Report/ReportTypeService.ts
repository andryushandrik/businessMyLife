...
export default class ReportTypeService {
	public static async paginate(
		config: PaginateConfig<ReportType>,
		filter?: ReportTypeFilterValidator['schema']['props'],
	): Promise<ModelPaginatorContract<ReportType>> {
	...
	public static async getAll(type: 'user' | 'offer'): Promise<ReportType[]> {
		...
		if (type === 'user') query = query.withScopes((scopes) => scopes.getByForUsers())
		else if (type === 'offer') query = query.withScopes((scopes) => scopes.getByForOffers())
	...
	private static filter(
		query: ModelQueryBuilderContract<typeof ReportType>,
		payload: ReportTypeFilterValidator['schema']['props'],
	): ModelQueryBuilderContract<typeof ReportType> {
