...
export function getUserEmailRules(
	withUniqueOrExists: 'unique' | 'exists' | false = false,
	currentUserId: User['id'] | null = null,
	table: string = TABLES_NAMES.USERS,
): Rule[] {
...
export function getUserPasswordRules(isWithConfirm = false): Rule[] {
	const rulesArr: Rule[] = [rules.containNumber(), rules.minLength(USER_PASSWORD_MIN_LENGTH), rules.maxLength(USER_PASSWORD_MAX_LENGTH)]

	if (isWithConfirm) rulesArr.push(rules.confirmed('passwordConfirm'))
...
export function getUserFirstNameRules(): Rule[] {
	return [rules.minLength(USER_FIRST_NAME_MIN_LENGTH), rules.maxLength(USER_FIRST_NAME_MAX_LENGTH)]
}

export function getUserLastNameRules(): Rule[] {
	return [rules.minLength(USER_LAST_NAME_MIN_LENGTH), rules.maxLength(USER_LAST_NAME_MAX_LENGTH)]
}

export function getUserPatronymicRules(): Rule[] {
	return [rules.minLength(USER_PATRONYMIC_MIN_LENGTH), rules.maxLength(USER_PATRONYMIC_MAX_LENGTH)]
}
