...
import {
  getUserFirstNameRules, getUserLastNameRules,
  getUserPatronymicRules, getUserCompanyNameRules,
...
export default class UserValidator extends IndexValidator {
  ...
  public schema = schema.create({
    ...
    phone: schema.string({ trim: true }, [
      ...
      rules.unique({ table: TABLES_NAMES.USERS, column: 'phone', whereNot: { id: this.currentUserId } })
    ]),
    ...
    mainStateRegistrationNumber: schema.number.optional([
      ...
      rules.unique({ table: TABLES_NAMES.USERS, column: 'mainStateRegistrationNumber', whereNot: { id: this.currentUserId } }),
    ]),
    taxpayerIdentificationNumber: schema.number.optional([
      rules.requiredWhen('type', 'in', [`${UserTypeNames.INDIVIDUAL_ENTREPRENEUR}`, `${UserTypeNames.LIMITED_LIABILITY_COMPANY}`]),
      rules.unique({ table: TABLES_NAMES.USERS, column: 'taxpayerIdentificationNumber', whereNot: { id: this.currentUserId } }),
    ]),
  })
