...
export const UploadTutorialFactory = Factory.define(UploadTutorial, ({ faker }) => {
	return {
		isEmbed: true,
...
export const PromoCodesFactory = Factory.define(PromoCode, ({ faker }) => {
	return {
...
export const ReportFactory = Factory.define(Report, async ({ faker }) => {
	return {
...
export const ReportTypesFactory = Factory.define(ReportType, ({ faker }) => {
	return {
...
export const AreaFactory = Factory.define(Area, ({ faker }) => {
	return { name: faker.helpers.unique(faker.lorem.word) }
})
	.relation('subsections', () => SubsectionFactory)
	.build()

export const SubsectionFactory = Factory.define(Subsection, ({ faker }) => {
	return { name: faker.helpers.unique(faker.lorem.word) }
}).build()

export const OfferFactory = Factory.define(Offer, async ({ faker }) => {
	...
	return {
		...
		category: faker.datatype.number({
			min: OfferCategories.SEARCH_FOR_INVESTORS,
		...
		projectStage: faker.datatype.number({
			min: OfferProjectStages.IDEA,
			max: OfferProjectStages.COMPLETE,
		}),
...
export const OfferImageFactory = Factory.define(OfferImage, ({ faker }) => {
...
export const UserFactory = Factory.define(User, ({ faker }) => {
	return {
		...
		type: faker.datatype.number({
			min: UserTypeNames.PHYSICAL_PERSON,
			max: UserTypeNames.LIMITED_LIABILITY_COMPANY,
		}),
		...
		experienceType: faker.datatype.number({
			min: UserExperienceTypes.BEFORE_THREE_MONTH,
			max: UserExperienceTypes.AFTER_THREE_YEARS,
		}),
...
export const UserImageFactory = Factory.define(UserImage, ({ faker }) => {
