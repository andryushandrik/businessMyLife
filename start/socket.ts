...
WebSocketService.io.on('connection', async (socket) => {
	...
	socket.on(
		...
		async (payload: any, cb: (response: Err | ResponseService) => void) => {
	...
	socket.on(
		'conversation:get',
		async (conversationId: Conversation['id'], cb: (response: Err | ResponseService) => void) => {
			const currentUserId: User['id'] = socket.data.userId!
			const conversation: Conversation | void = await ConversationsController.get(
				conversationId,
				currentUserId,
				cb,
			)
	...
	socket.on(
		'message:create',
		async (payload: any, cb: (response: Err | ResponseService) => void) => {
			const data: ReturnMessageCreatePayload | void = await MessagesController.create(
				socket.data.userId!,
				payload,
				cb,
			)
				...
				for (const item of usersIds) {
					...
					if (someSocket) {
						...
						socket.to(someSocket.socketId).emit(
							'conversation:update',
							await ConversationService.get(data.conversation.id, {
	...
	socket.on(
		...
		async (toId: User['id'], payload: any, cb: (response: Err | ResponseService) => void) => {
			const createPayload: MessageCreateWithoutTopicPayload = {
				toId,
				fromId: socket.data.userId!,
			}

			const data: ReturnMessageCreatePayload | void = await MessagesController.createWithoutTopic(
				payload,
				createPayload,
				cb,
			)
	...
	socket.on(
		...
		async (toId: User['id'], payload: any, cb: (response: Err | ResponseService) => void) => {
			...
			const data: ReturnMessageCreatePayload | void = await MessagesController.createWithOfferTopic(
				payload,
				createPayload,
				cb,
			)
	...
	async function sendMessageViewedForMessageCreator(
		conversationId: Conversation['id'],
		userId: User['id'],
	): Promise<void> {
		const someSocket: SomeSocket | undefined = ALL_SOCKETS.getSocket(userId)
		const allSockets: Set<string> = await socket
			.in(getConversationRoomName(conversationId))
			.allSockets()
